...
int main() {
    ...
    doCardThrowSort(myList);
    
    return 0;
}


void doCardThrowSort(List<int> &list) {
    List<int> testList;
    testList = list;

    cout << "Before Card Throw Sort: " << endl;
    printList(testList);

    //do the sort
    cardThrowSort(testList);   
    
    cout << endl << "After Card Throw Sort: " << endl;
    printList(testList);
    if (!isSorted(testList))
        cout << "***** List is NOT sorted!" << endl;
    if (!sameEntries(testList, list))
        cout << "****ENTRIES HAVE BEEN MODIFIED!" << endl;
}
